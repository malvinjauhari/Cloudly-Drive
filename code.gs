...
function saveWorker(originalId, data) {
  ...
  if (originalId) {
    ...
    // ADD new worker - check duplicate
    for (let i = 1; i < existing.length; i++) {
    ...
    // Try to sync quota from worker endpoint
    ...
    // Append row: id, name, email, url, status, quota, used, free
...
// ==========================================
// 8. SETTINGS: CATEGORIES (legacy compat)
// ==========================================
...
// ==========================================
// 9. ACTIVITY LOGS & CATEGORY DELETE
// ==========================================
...
// 11. UTILITIES
// ==========================================
function logToDatabase(sheetName, rowData) { SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID).getSheetByName(sheetName).appendRow(rowData); }
function byteToHex(bytes) { return bytes.map(b => ('0' + (b & 0xFF).toString(16)).slice(-2)).join(''); }
